...
CREATE VIEW 
viewdetalleventadvp AS 
select dv.iddetalleventa, dv.idfactura, p.nombre as idproducto, dv.cantidad, dv.precio, dv.total, dv.estado from  detalleventa dv
inner join producto p on p.idproducto = dv.idproducto;


CREATE VIEW 
viewpromocionespp AS 
select m.idpromociones, p.nombre, m.descuento, m.fechainicio, m.fechafin, m.descripcion, m.estado from promociones m
inner join producto p on p.idproducto = m.idproducto;
...
CREATE VIEW 
personapdt AS 
select persona.idpersona, persona.cedula, persona.nombres, persona.apellidos, persona.email, 
telefono.convencional, telefono.celular1, telefono.celular2, 
direccion.provincia, direccion.canton, direccion.parroquia, direccion.sector,
direccion.calleprincipal, direccion.numeracion, direccion.callesecundaria, direccion.descripcion from persona
inner join telefono on persona.idtelefono = telefono.idtelefono 
inner join direccion on persona.iddireccion = direccion.iddireccion;
...
p.idproducto, p.idcategoria, p.nombre, p.image, p.precio, m.descuento, p.stock, p.estado 
from promociones m 
inner join producto p on p.idproducto = m.idproducto;
...
CREATE VIEW 
viewpromocionesppuni AS 
select m.idpromociones, p.idproducto, p.idcategoria,
 p.nombre, p.image, p.precio, m.descuento, p.stock, p.estado
 from promociones m 
inner join producto p on p.idproducto = m.idproducto;